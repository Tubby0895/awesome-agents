...
## Testing Guidelines
There is no automated test framework yet. Validate contributions by checking that each new agent folder contains both `README.md` and `SOUL.md`, that each category keeps a current `agents/<category>/README.md`, that `agents.json` is updated, and that the prompt reads coherently end to end. When changing multiple agents, spot-check category listings in the root [README.md](/Users/xubin/work/awesome-agents/README.md).
